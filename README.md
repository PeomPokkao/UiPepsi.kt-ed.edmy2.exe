...
local library = {
	...
	colors = {
		main = Color3.fromRGB(42, 38, 255),
		background = Color3.fromRGB(6, 8, 41),
		outerBorder = Color3.fromRGB(53, 67, 240),
		innerBorder = Color3.fromRGB(0, 178, 255),
		topGradient = Color3.fromRGB(35, 35, 35),
		bottomGradient = Color3.fromRGB(29, 29, 29),
		sectionBackground = Color3.fromRGB(24, 61, 82),
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./UpperCut Hub")
							if fold then
							else
								makefolder("./UpperCut Hub")
							end
							fold = isfolder("./UpperCut Hub/Themes")
							if fold then
							else
								makefolder("./UpperCut Hub/Themes")
							end
							fold = isfolder("./UpperCut Hub/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./UpperCut Hub Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./UpperCut Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./UpperCut Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./UpperCut Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "UpperCut Hub" then
		library.WorkspaceName = convertfilename(windowName, "UpperCut Hub")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./UpperCut Hub") then
						makefolder("./UpperCut Hub")
					end
					local common_string = "./UpperCut Hub/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./UpperCut Hub/" .. tostring(custom_workspace or library.WorkspaceName)
						end
